...
function sH(e, t) {
  l
    ? (e.clip.height = t)
    : op
    ? (e.style.pixelHeight = t)
    : (e.style.height = t + px);
}
function sC(e, t, i, o, s) {
  l
    ? ((X = e.clip), (X.top = t), (X.right = i), (X.bottom = o), (X.left = s))
    : (e.style.clip =
        "rect(" + t + px + " " + i + px + " " + o + px + " " + s + px + ")");
}
function wH(e, t) {
  l && ((Y = e.document), Y.open(), Y.write(t), Y.close()),
...
function Neko(x, y, active, imagedirectory) {
  (this.findHome = function () {
    ...
    window.remoteimages ||
      (imagedirectory = "https://webneko.net/" + imagedirectory),
  ...
  (this.whichNeko = aNekos.length),
    ...
    (this.a_resting = new Array(
      "",
      'this.SetBehavior("wakingup")',
      "this.chooseIdle()",
      "8",
      "1",
      "still"
    )),
    (this.a_itching = new Array(
      "",
      'this.SetBehavior("wakingup")',
    ...
    (this.a_yawning = new Array(
      "",
      'this.SetBehavior("wakingup")',
      'this.SetBehavior("resting2")',
      "5",
      "1",
      "yawn"
    )),
    (this.a_resting2 = new Array(
      "",
      'this.SetBehavior("wakingup")',
  ...
  (this.layer = createLayer(strLayer, x, y, 32, 32, strContent)),
    ...
    window.delay && window.delay > 0
      ? (this.delay =
          window.delay + Math.floor(nDelayVariance * Math.random() - 5))
      : (this.delay = 250 + Math.floor(nDelayVariance * Math.random() - 5)),
    (this.delayMultiplier = 1),
    window.stepsize && window.stepsize > 0
    ...
    (this.boardX = -1),
    (this.boardY = -1),
    (this.eccX = Math.floor(
      checkerboardEccentricity * Math.random() - checkerboardEccentricity / 2
    )),
    (this.eccY = Math.floor(
      checkerboardEccentricity * Math.random() - checkerboardEccentricity / 2
    )),
    this.Think();
}
function NekoMessage() {
  var e = this.caught ? sNekoMessageCaught : sNekoMessage;
  window.status = e;
}
...
function NekoCalculateDistance() {
  (this.distx = this.endx - this.layer.myx),
    ...
    (this.steps =
      Math.sqrt(Math.pow(this.distx, 2) + Math.pow(this.disty, 2)) /
      this.stepsize),
    this.steps >= 1
      ? (this.caught && eval(this.onUnCaught), (this.caught = !1))
      : (this.caught || eval(this.onCaught), (this.caught = !0)),
...
function NekoUpdateImage() {
  ...
  if (this.frame >= paImages.length)
    if (
      ...
      0 != this.loopTimes && this.behaviorRepetition >= this.loopTimes)
    ) {
...
var checkerboardEccentricity = 10,
  ...
  nDelayVariance = 20,
  nFirstRealFrame = 5,
  sNekoMessage = (sNekoMessageCaught = sNekoMessage),
  aNekos = new Array();
...
for (
  var checkerboard = new Array(checkerboardScale), i = 0;
  i < checkerboardScale;
  i++
) {
